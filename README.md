...
| `perconalab/percona-toolkit` イメージ | `pt-*` コマンドが入った別コンテナ。DB と同居しない構成にしている |
...
- ツール実行用に **DB と分離した** `perconalab/percona-toolkit` を別コンテナで常駐。「pt-* は DB と同居しない」という運用上の自由度をそのまま再現
