...
- **slowlog の rotate**: 本番では `mysqladmin flush-logs` で切ってから集計するのが安全 (集計中に書き込み続けると end of file 判定でズレる)
- **`long_query_time=0` は I/O 負荷**: 本番で常時 ON はやめる。サンプリング期間だけ
- **個人情報を含むクエリ**: ログに WHERE 句の値が生で乗る。フィンガープリント (`?` 化) は集計時の話で、生ログ自体は要注意
