...
echo
PHONE_EXISTS=$($SOURCE -N -B -e "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='shop' AND TABLE_NAME='users' AND COLUMN_NAME='phone';" 2>/dev/null)
...
if [[ "$PHONE_EXISTS" -eq 0 ]]; then
  ...
  (
    for i in $(seq 1 20); do
      $SOURCE -e "INSERT INTO shop.users (email, name) VALUES ('osc-${RUN_ID}-$i@example.com','OSC user $i');" 2>/dev/null
      sleep 0.5
    done
  ) &
  BG_PID=$!
  ...
  echo "== phone カラムは既に存在するため pt-online-schema-change をスキップ =="
fi
